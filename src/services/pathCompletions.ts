...
namespace ts.Completions.PathCompletions {
    ...
    function createPathCompletion(name: string, kind: NameAndKind["kind"]): NameAndKind {
        return { name, kind };
    }
    ...
    export function getStringLiteralCompletionsFromModuleNames(sourceFile: SourceFile, node: LiteralExpression, compilerOptions: CompilerOptions, host: LanguageServiceHost, typeChecker: TypeChecker): PathCompletion[] {
        ...
        return getStringLiteralCompletionsFromModuleNamesWorker(node, compilerOptions, host, typeChecker).map(n => ({ ...n, span }));
    }

    function getStringLiteralCompletionsFromModuleNamesWorker(node: LiteralExpression, compilerOptions: CompilerOptions, host: LanguageServiceHost, typeChecker: TypeChecker): NameAndKind[] {
        ...
        if (tryDirectoryExists(host, baseDirectory)) {
            ...
            if (files) {
                ...
                forEachKey(foundFiles, foundFile => {
                    result.push(createPathCompletion(foundFile, ScriptElementKind.scriptElement));
                });
            }
            ...
            if (directories) {
                for (const directory of directories) {
                    ...
                    result.push(createPathCompletion(directoryName, ScriptElementKind.directory));
                }
            }
        }
    ...
    function getCompletionEntriesForNonRelativeModules(fragment: string, scriptPath: string, compilerOptions: CompilerOptions, host: LanguageServiceHost, typeChecker: TypeChecker): NameAndKind[] {
                ...
                if (paths.hasOwnProperty(path) && patterns) {
                    for (const { name, kind } of getCompletionsForPathMapping(path, patterns, fragment, baseUrl, fileExtensions, host)) {
                        ...
                        if (!result.some(entry => entry.name === name)) {
                            result.push(createPathCompletion(name, kind));
                        }
                    }
                }
            }
        ...
        for (const moduleName of enumeratePotentialNonRelativeModules(fragment, scriptPath, compilerOptions, typeChecker, host)) {
            result.push(createPathCompletion(moduleName, ScriptElementKind.externalModuleName));
        }
    ...
    export function getTripleSlashReferenceCompletion(sourceFile: SourceFile, position: number, compilerOptions: CompilerOptions, host: LanguageServiceHost): NameAndKind[] | undefined {
        ...
        const [, , kind, toComplete] = match;
        const scriptPath = getDirectoryPath(sourceFile.path);
        switch (kind) {
            case "path":
                // Give completions for a relative path
                return getCompletionEntriesForDirectoryFragment(toComplete, scriptPath, getSupportedExtensions(compilerOptions), /*includeExtensions*/ true, host, sourceFile.path);
            case "types":
                // Give completions based on the typings available
                return getCompletionEntriesFromTypings(host, compilerOptions, scriptPath);
            default:
                return undefined;
        }
    }
    ...
    function getDirectoryFragmentTextSpan(text: string, textStart: number): TextSpan | undefined {
        ...
        // If the range is an identifier, replacementSpan is unnecessary.

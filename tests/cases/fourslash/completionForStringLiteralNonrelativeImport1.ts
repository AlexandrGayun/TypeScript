...
["import_as", "import_equals", "require"].forEach((kind, i) => {
    verify.completionsAt(`${kind}0`, ["fake-module", "fake-module-dev"]);
    verify.completionsAt(`${kind}1`, ["dts", "index", "ts", "tsx"]);
    verify.completionsAt(`${kind}2`, ["fake-module", "fake-module-dev"].map(name => ({ name, replacementSpan: test.ranges()[i] })));
});
